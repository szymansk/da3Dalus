...
class myDisplay:
    ...
    def instance():
        if myDisplay.my_instance is None:
            myDisplay.my_instance= myDisplay(5, True)
        return myDisplay.my_instance

    def display_this_shape(self,shape,trans=False):
        if self.dev:           
            self.id+=1
            ...
            if trans:
                ...
                self.display.DisplayMessage(point=Ogp.gp_Origin(), text_to_write="0 Punkt")
            else:
                self.display.DisplayShape(shape)
            self.display.FitAll()
            print(str(self.id), " New Position= " , str(self.position))
           # except:
               # logging.warning("Shape can not be displayed: posible Null")
