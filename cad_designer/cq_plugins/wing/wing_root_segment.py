...
def wing_root_segment(self: cq.Workplane, root_airfoil: str, root_chord: float, tip_chord: float, length: float,
                      sweep: float = 0, sweep_mode: Literal["distance", "angle"] = "distance",
                      root_incidence: float = 0, tip_incidence: float = 0, root_dihedral: float = 0,
                      tip_dihedral: float = 0, tip_airfoil: str = None, offset: float = 0,
                      number_interpolation_points: int = None, root_plane: Plane = None, tip_plane: Plane = None,
                      connected:bool =True):
    ...
    final_wing: Workplane = airfoil_tip.loft(combine='a', ruled=True) #--> airfoils must have same number of points
    ...
    if connected:
        ...
        final_wing = final_wing.union(toUnion=center_wing).copyWorkplane(self).split(keepBottom=True)

    return final_wing.newObject([tip_plane, airfoil_tip.val(), final_wing.val()])
