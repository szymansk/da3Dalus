...
if __name__ == "__main__":
    ...
    servo_information_a = ServoInformation(height=0.023, width=0.012, length=0.024, lever_length=0.018, rot_x=90)
    servo_pt, servo_ang = servo_information_a.place_elevator_servo(tigl_h, 1, 2, 0)

    # defining as simple root node
    root_node = ConstructionRootNode(creator_id="fuselage")
    ...
    wing_attachment_bolt_node = ConstructionStepNode(
        WingAttachmentBoltCutoutShapeCreator("attachment_bolts", fuselage_loft="full_fuselage_loft",
    ...
    servo_shape_import = ConstructionStepNode(
        IgesImportCreator("servo", iges_file="../components/servos/unknown/Servo.iges", trans_x=servo_pt.X(),
                          trans_y=servo_pt.Y(), trans_z=servo_pt.Z(), rot_x=.0, rot_y=90.0, rot_z=-90.0 + servo_ang,
                          scale=0.001))
    root_node.append(servo_shape_import)

    stamp_shape_import = ConstructionStepNode(
        IgesImportCreator("servo_stamp", iges_file="../components/servos/unknown/servo_stamp.iges",
                          trans_x=servo_pt.X(), trans_y=servo_pt.Y(), trans_z=servo_pt.Z(), rot_x=.0, rot_y=90.0,
                          rot_z=-90.0 + servo_ang, scale=0.001))
    ...
    engine_cape_node.append(wing_support_node)
    ...
    fuse_fuselage_reinforcements = ConstructionStepNode(
        FuseMultipleShapesCreator("fuselage_reinforcement",
                                  shapes=["fuselage_reinforcement",
                                          "servo",
                                          "wing_support",
                                          "elevator_support",
                                          ]))
    ...
    fuse_servo_with_final_fuselage_node.append(cut_wing_from_fuselage_node)
