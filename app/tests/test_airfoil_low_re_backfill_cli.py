...
def test_backfill_skips_already_computed_airfoils(backfill_db):
    ...
    with backfill_db() as session:
        af = AirfoilModel(name="sd7037_backfill_test", coordinates=[[0, 0], [1, 0]])
        session.add(af)
        session.flush()
    ...
    with patch("app.services.airfoil_low_re_service.compute_airfoil_low_re", return_value=[]) as mock_compute:
...
def test_backfill_processes_uncovered_airfoils(backfill_db):
    ...
    with patch("app.services.airfoil_low_re_service.compute_airfoil_low_re", return_value=[fake_polar]) as mock_compute:
        from scripts.backfill_airfoil_low_re import run_backfill
        with backfill_db() as session:
            run_backfill(session=session, force=False)
...
def test_backfill_force_flag_recomputes_all(backfill_db):
    ...
    with backfill_db() as session:
        af = AirfoilModel(name="force_recompute_af", coordinates=[[0, 0], [1, 0]])
        session.add(af)
        session.flush()
        session.add(AirfoilLowRePolarModel(
...
def test_backfill_logs_progress(backfill_db, caplog):
    ...
    with backfill_db() as session:
        af = AirfoilModel(name="logged_af", coordinates=[[0, 0], [0.5, 0.06], [1, 0]])
        session.add(af)
        session.commit()
