...
def test_low_re_airfoil_scores_higher_than_transonic():
    ...
    # Compute polars for SD7037
    sd7037_polars = compute_airfoil_low_re(
        "sd7037_test",
        np.array(_SD7037_COORDS),
        re_test,
        model_size="large",  # use 'large' for speed in slow test
    ...
    assert sd7037_score is not None, "SD7037 re_agnostic score is None"
    assert rae2822_score is not None, "RAE2822 re_agnostic score is None"

    assert sd7037_score > rae2822_score, (
        f"Expected SD7037 (low-Re optimised) to score higher than RAE2822 (transonic) "
        f"at Re=100k, but got SD7037={sd7037_score:.3f}, RAE2822={rae2822_score:.3f}"
    )
