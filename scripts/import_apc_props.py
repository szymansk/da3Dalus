...
Reads the versioned factual snapshot (data/cots/apc_props.json by default) and
...
Options:
    --snapshot PATH   Path to apc_props.json (default: data/cots/apc_props.json)
...
from app.services.prop_polar_import import import_prop_polars  # noqa: E402

DEFAULT_SNAPSHOT = REPO_ROOT / "data" / "cots" / "apc_props.json"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=f"Path to apc_props.json (default: {DEFAULT_SNAPSHOT})",
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    if args.dry_run:
        ...
        for r in records:
            ...
            print(
                f'  {r["name"]:20s}  diameter={r["specs"].get("diameter_in")}"  '
