#!/usr/bin/env python3
"""Import carbon-fibre tube stock from a versioned JSON snapshot (gh-1081).

Usage:
    poetry run python scripts/import_carbon_tubes.py [SNAPSHOT] [--force]

Arguments:
    SNAPSHOT    Path to the JSON snapshot (default: data/cots/carbon_tubes.json)
    --force     Overwrite existing rows even if they appear unchanged

Mirrors scripts/import_cots.py: single transaction, result report, no network
required.  The committed snapshot is the durable reimport source.

After running:
    - Restart the backend (uvicorn does not hot-reload the DB state).
    - Re-run after upgrading to a new snapshot version to update existing rows.
"""
...
DEFAULT_SNAPSHOT = REPO_ROOT / "data" / "cots" / "carbon_tubes.json"
