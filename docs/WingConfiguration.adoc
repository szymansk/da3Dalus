...
The +WingConfiguration+ is important for the generation of an aeroplane. As it defines the shape and function of the wing, rudder, elevator or other wing like shapes.

In general it consists of an ordered list of segments. The list always starts with one _root_-segment, followed by a number of _segment_s wich are followed by a number of _tip_-segments.
...
Before lofting the airfoils they can be rotated around the _chord line_ which leads to a dihedral of the wing. Or it can be rotated around a _rotation point (defined) relative (to the) chord (length)_ we denote the chord length with latexmath:[C_{i}] and the relative chord factor with latexmath:[rc_{i} \in (0,1)], which is called the _angle of incidence (AoI)_ denoted with latexmath:[\iota_{i}]. See figures xref:airfoil_parameter[xrefstyle=short], xref:airfoil_rot0[xrefstyle=short] and xref:airfoil_rot03[xrefstyle=short].
...
===== Tip
The tip is a special segment, which is also defined like the previous segment, but it also take the parameter _tip type_, which defines how the tip will be designed. Currently we only support 'flat' and 'round'. 'flat' is just like a flat wall attached wing tip. 'round' tries to use a chamfer to round the outlines of the airfoil. A tip does not have a spare nor a trailing edge device.

==== Segment Coordinate Systems
The coordinate system for a segment is defined by the previously defined parameters. It is always lokated in the nose point. We define it in a way, that it starts like the airplanes coordinate systems. Therefor x-axis goes from the leading edge nose point to the trailing edge end point. The y-axis points along the wing and the z-axis is pointing upwards along the produced lift vector of the segment.
...
latexmath:[H_0 = T(C_{0} rc_{0}, 0,0)\cdot R_x(\delta_{0})\cdot R_y(\iota_{0}) \cdot T(-C_{0} r_{0}, 0,0)]

latexmath:[H_i = T(C_{i} rc_{i}, 0, 0)\cdot T(S_{i-1}, L_{i-1},0) \cdot R_x(\delta_i) \cdot R_y(\iota_i) \cdot T(-C_{i} rc_{i}, 0,0)]
...
latexmath:[H_i = \left[
\begin{matrix}
\cos{\left(\iota_i \right)} & 0 &\sin{\left(\iota_i \right)}  & - C_i rc_i \cos{\left(\iota_i \right)} + C_i rc_i + S_{i-1}\\
\sin{\left(\delta_i \right)} \sin{\left(\iota_i \right)} & \cos{\left(\delta_i \right)} & - \sin{\left(\delta_i \right)} \cos{\left(\iota_i \right)} & - C_i rc_i \sin{\left(\delta_i \right)} \sin{\left(\iota_i \right)} + L_{i-1}\\
- \sin{\left(\iota_i \right)} \cos{\left(\delta_i \right)} & \sin{\left(\delta_i \right)} & \cos{\left(\delta_i \right)} \cos{\left(i \right)} & C_i rc_i \sin{\left(\iota_i \right)} \cos{\left(\delta_i \right)}\\
0 & 0 & 0 & 1
\end{matrix}
\right\]]
...
latexmath:[ \iota_i = atan2(\sin(\iota_i), \cos(\iota_i)) = atan2(H_i[0,2\], H_i[0,0\])]

Dihedral in radians:

latexmath:[ \delta_i = atan2(\sin(\delta_i), \cos(\delta_i)) = atan2(H_i[2,1\], H_i[1,1\])]

Relative rotation point of incidence angle:

latexmath:[ rc_i = H_i[3,3\]/C_i \sin{\left(\iota_i \right)} \cos{\left(\delta_i \right)}]

Segment length in length units:

latexmath:[ L_{i-1}  = H_i[1,3\] + C_i rc_i \sin{\left(\delta_i \right)} \sin{\left(\iota_i \right)} = H_i[1,3\] + C_i rc_i H_i[1,0\]]

Sweep in length units:

latexmath:[ S_{i-1} = H_i[0,3\] + C_i rc_i \left(\cos{\left(\iota_i \right)} - 1\right) =  H_i[0,3\] + C_i rc_i \left(H_i[0,0\] - 1\right)]

And from this the sweep in radians:

latexmath:[s_{i-1} = atan2\left({S_{i-1}, L_{i-1}}\right)]


==== Spare
.The main spare.
image::../images/images with text/Folie7.png[spare, id=spare]

A spare is a construction that runs lengthwise through the wing. We can add arbitrary spares to a wing. However, the spare with index 0, will always be treated as the main spare and has a major impact on the construction of the wing. In general this spare takes all the load of the wing and to reduce wing twist it should be placed in the area of the largest lift, which is in normal flight around the point of the maximum thickness, which is in most cases between 25%-30% of the chord length. You can define this placement via the _spare position factor_, the default value is 0.25.

Another way to overwrite this setting is to use the _spare origin_. This is a vector given in the segments coordinate system which defines the spare's placement.

An other important vector is the _spare vector_ it is also given in the segment's coordinate system and defines in which direction the spare will be lofted.

The _spare length_ of spare tells us how far the spare will be lofted along the _spare vector_ and the _spare start_ states at which point on the _spare vector_ the loft will start.

In general our spares will have some kind of support structure, which is not printed. This usually is a carbon rod round or rectangular formed. You can give the dimension of the rod by the _spare support dimension height_ and _spare support dimension width_ if height is equal width we will interpret this as a round rod. Otherwise it is rectangular.

NOTE: You can create a squared rod by adding latexmath:[1e^{-10}] on one of the dimensions.

===== Spare Mode
The most important parameter is the _spare mode_ as it helps you to define some standard spares, which will calculate the _spare vector_ and the _spare origin_ for you if you set them to 'None'. If one of the following modes is effected by a previous spare or effects previous spares, than it will only effect spares with the same index.

====== normal
The *normal* _spare vector_ will be set to be normal to the xz-plane of the segment's coordinate system. It is equal to the segment's coordinate systems y-axis vector.

====== follow
The _spare vector_ will *follow* the direction of the previous one, so the _spare vector_ will be the previous _spare vector_ expressed in the current segment's coordinate system.

====== standard
The *standar* _spare vector_ is calculated in that way, that the spare vector will point from the mean camber point at the given relative chord (_spare position factor_) of the _root airfoil_ to the corresponding point of the _tip airfoil_. The start point also defines the _spare origin_ if it is not overwritten.

====== standard backward
This mode is similar to the previous one. But if differs in the way, how the _root airfoil_ is selected. If we come to a spare in 'standard_backward' mode, we backtrace through the previous spares and select the first spare, that is not in 'follow' mode. We use this segments _root airfoil_ in the same manner as in the 'standard' case. After calculation of the origin and the spare vector we adjust the other segments in between.

NOTE: We also use (_spare position factor_) of the spare we found during backtracking.

====== orthogonal backward
This mode is similar to the previous one. However, the spare will be orthogonal to the XZ-plane.

==== Trailing Edge Device
.A trailing edge device, here an aileron.
image::../images/images with text/Folie8.png[spare, id=spare]
...
image::../images/images with text/Folie9.png[spare, id=servo]
