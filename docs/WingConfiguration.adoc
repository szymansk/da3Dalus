...
- \sin{\left(\iota_i \right)} \cos{\left(\delta_i \right)} & \sin{\left(\delta_i \right)} & \cos{\left(\delta_i \right)} \cos{\left(\iota_i \right)} & C_i rc_i \sin{\left(\iota_i \right)} \cos{\left(\delta_i \right) + D_{i-1}}\\
0 & 0 & 0 & 1
\end{matrix}
\right\]]
...
latexmath:[D_{i-1} = H_i[3,3\] - 0.25 \cdot C_i \sin{\left(\iota_i \right)}]
