...
```bash
cd frontend
npm run test:e2e -- analysis-no-field-length
```
...
- [ ] **Step 5.1: Start dev server (if not already running)**
...
The Playwright `webServer` config sets `reuseExistingServer: true`, so a server left over from the BDD run is fine.

- [ ] **Step 5.2: Visit the Analysis page and inspect**

Open `http://localhost:3000/workbench`, pick an aeroplane, click the Analysis step pill.
...
Open `http://localhost:3000/workbench`, click the Mission step pill.
