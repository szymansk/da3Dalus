...
### Architecture Decision Records — binding

`_reversa_sdd/adrs/` holds the ADRs governing this codebase. **They are binding,
and a violation is a blocking review finding.**

How they are used — the **planner** decides conformance, implementers only
execute:

1. Scan the index `_reversa_sdd/adrs/README.md` (one line per ADR).
2. Read in full **only** the ADRs the change touches.
3. Record a **`## Binding ADRs`** section in the plan, naming per ADR the
   concrete constraint on *this* feature — not the general rule. Implementers
   inherit the constraints through the plan and need no ADR access of their own.

Always applicable, on every change:
...
- **ADR 0020** — no *undeclared* fallbacks: every substitution, clamp, retry or
  truncation emits a `DesignWarning` whose `severity` says whether it is domain
  practice or a defect.
- **ADR 0021** — complete but unreachable code is deleted by default; "inert" is
  forbidden.
- **ADR 0023** — engineering constants carry their source and are validated at
  **RC/UAV scale (0.5–15 kg)**, never adopted because they are standard in
  transport-category literature.

If a change is right and an ADR is wrong, the resolution is a **new ADR
superseding it** — never an undocumented exception.
