...
class FuselageCutouts:
    ...
    def create_cylinder_pattern(cls, radius: float, height: float, quantity: float,
        ...
        :return:
        '''
        cylinder = cq.Workplane(OPrim.BRepPrimAPI_MakeCylinder(radius, height).Shape(), "cylinder_cutout")
    ...
    def create_hardware_cutout(cls, fuselage_dimensions: ShapeDimensions, wing_dimensions: ShapeDimensions,
                               width_factor, length_factor, position="bottom") -> cq.Workplane:
        """
        ...
        moved_box = cq.Workplane(OExs.translate_shp(box,
        ...
        cylinder_front = cq.Workplane(OExs.translate_shp(cylinder, Ogp.gp_Vec(hardware_x_pos, 0.0, hardware_z_pos)),
                                          "c1_cutout")
        hardware_x_pos += hardware_cutout_length
        cylinder_back = cq.Workplane(OExs.translate_shp(cylinder, Ogp.gp_Vec(hardware_x_pos, 0.0, hardware_z_pos)),
                                         "c2_cutout")
    ...
    @classmethod
    def create_bolt_hole(cls, overlap_dimensions: ShapeDimensions) -> cq.Workplane:
        ...
        #bolt_hole = cq.Workplane(cylinder, "bolt_hole")
        ...
        bolt_holes = cq.Workplane('XZ').moveTo(x_pos, z_pos).cylinder(height=cylinder_lenght, radius=6/2, direct=(0, 1, 0)).display()
        return bolt_holes
